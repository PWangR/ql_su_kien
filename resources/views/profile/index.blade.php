...
<style>
    ...
    .profile-banner {
        ...
        border: 1px solid var(--border);
        background:
            linear-gradient(120deg, rgba(59, 81, 123, 0.12), rgba(255, 255, 255, 0.92)),
    ...
    <aside class="profile-panel">
        ...
        <div class="profile-side-body">
            ...
            <div class="profile-side-section">
                ...
                @if($nextEvent && $nextEvent->suKien)
                <div class="profile-security-item">
                    <strong>{{ $nextEvent->suKien->ten_su_kien }}</strong>
                    <p>{{ $nextEvent->suKien->thoi_gian_bat_dau?->format('d/m/Y H:i') }} · {{ $nextEvent->suKien->dia_diem }}</p>
                </div>
                @else
                <div class="profile-empty">
                    Bạn chưa có sự kiện sắp tới. Hãy khám phá danh sách sự kiện mới để đăng ký tham gia.
                </div>
                @endif
            </div>
        </div>
    ...
    <section class="profile-main">
        <div class="profile-banner">
            <div>
                <h2>Hồ sơ cá nhân</h2>
                <p>
                    Trang hồ sơ hiển thị thông tin cá nhân, học tập và hoạt động một cách rõ ràng,
                    giúp người dùng dễ dàng theo dõi và quản lý quá trình tham gia của mình trong hệ thống.

                </p>
            </div>

            <div class="profile-quick-pills">
                <div class="profile-quick-pill">
                    <i class="bi bi-envelope-check"></i>
                    {{ $user->hasVerifiedEmail() ? 'Email đã xác thực' : 'Email chưa xác thực' }}
                </div>
                <div class="profile-quick-pill">
                    <i class="bi bi-award"></i>
                    {{ (int) $totalPoints }} điểm tích lũy
                </div>
                <div class="profile-quick-pill">
                    <i class="bi bi-clock-history"></i>
                    {{ (int) ($registrationStats->total ?? 0) }} lượt đăng ký
                </div>
            </div>
        </div>
        ...
        <div class="profile-grid">
                    ...
                    <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data">
                        ...
                        <div class="profile-form-grid">
                            <div class="form-group">
                                ...
                                <input type="text" value="{{ $user->ma_sinh_vien }}" disabled class="form-control profile-readonly">
                            </div>

                            <div class="form-group">
                                <label class="form-label">Email</label>
                                <input type="text" value="{{ $user->email }}" disabled class="form-control profile-readonly">
                                <small class="profile-helper">Email đăng nhập hiện không thể chỉnh sửa tại đây.</small>
                            </div>

                            <div class="form-group">
                                <label class="form-label">Họ và tên *</label>
                                <input type="text" name="ho_ten" value="{{ old('ho_ten', $user->ho_ten) }}" required class="form-control @error('ho_ten') is-invalid @enderror">
                                @error('ho_ten')
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="lop" value="{{ old('lop', $user->lop) }}" required class="form-control @error('lop') is-invalid @enderror" placeholder="VD: 64.CNTT-1">
                                <small class="profile-helper">Định dạng chuẩn: số.chữ-số, ví dụ 64.CNTT-1.</small>
                                @error('lop')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="so_dien_thoai" value="{{ old('so_dien_thoai', $user->so_dien_thoai) }}" class="form-control @error('so_dien_thoai') is-invalid @enderror" placeholder="VD: 0912345678">
                                @error('so_dien_thoai')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="file" name="avatar" accept="image/*" class="form-control @error('avatar') is-invalid @enderror" style="border-style:dashed;">
                        ...
                        <div class="profile-submit-row">
                            <p class="profile-submit-note">
                                Mọi thay đổi được lưu trực tiếp vào hồ sơ hiện tại của bạn. Nếu đổi mật khẩu, hãy dùng mật khẩu mới cho lần đăng nhập tiếp theo.
                            </p>
            ...
            <div class="profile-main" style="gap:24px;">
                <div class="profile-panel">
                    <div class="profile-card-head">
                        <div>
                            <h3>Bảo mật và trạng thái</h3>
                            <p>Các tín hiệu nhanh về tài khoản của bạn trong hệ thống.</p>
                        </div>
                    </div>
                    ...
                    <div class="profile-card-body">
                        <div class="profile-security-list">
                            <div class="profile-security-item">
                                ...
                                <p>{{ $user->hasVerifiedEmail() ? 'Email của bạn đã được xác thực và có thể sử dụng đầy đủ chức năng.' : 'Email của bạn chưa được xác thực. Một số thao tác đăng nhập có thể bị giới hạn.' }}</p>
                            </div>
                            <div class="profile-security-item">
                                <strong>Trạng thái tài khoản</strong>
                                <p>Tài khoản hiện ở trạng thái <strong>{{ $statusLabel }}</strong>. Vai trò của bạn là <strong>{{ $roleLabel }}</strong>.</p>
                            </div>
                            <div class="profile-security-item">
                                <strong>Thông tin liên hệ</strong>
                                <p>{{ $user->so_dien_thoai ? 'Bạn đã cập nhật số điện thoại liên hệ.' : 'Bạn chưa cập nhật số điện thoại, nên bổ sung để hồ sơ đầy đủ hơn.' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="profile-card-body">
                        ...
                        <div class="profile-activity-list">
                            ...
                            <div class="profile-activity-item">
                                ...
                                <p>
                                    Đăng ký lúc {{ \Carbon\Carbon::parse($registration->thoi_gian_dang_ky)->format('d/m/Y H:i') }}
                                    @if($registration->suKien)
                                    · {{ $registration->suKien->dia_diem }}
                                    @endif
                                </p>
                                <div class="profile-activity-meta">
                                    <span class="badge badge-{{ ($registration->trang_thai_tham_gia === 'da_tham_gia') ? 'success' : (($registration->trang_thai_tham_gia === 'huy') ? 'danger' : 'primary') }}">
                        ...
                        @else
                        <div class="profile-empty">
                            Chưa có lịch sử đăng ký nào để hiển thị. Khi bạn tham gia sự kiện, thông tin sẽ xuất hiện tại đây.
                        </div>
                        @endif
                    </div>
                </div>
