...
<div style="display:grid;grid-template-columns:1fr 320px;gap:24px;align-items:start;">
    ...
    <div>
        ...
        <div class="info-box">
            <h3 style="font-size:15px;font-weight:700;margin-bottom:16px;"><i class="bi bi-images" style="color:#2563eb;"></i> Hình ảnh sự kiện</h3>
            <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:12px;">
                @foreach($suKien->media as $img)
                <a href="{{ asset('storage/'.$img->duong_dan_tep) }}"
                    data-lightbox="gallery-{{ $suKien->ma_su_kien }}"
                    style="position:relative;display:block;border-radius:12px;overflow:hidden;aspect-ratio:1;background:#f1f5f9;border:1px solid #e2e8f0;">
                    ...
                    <div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0);transition:background 0.2s;"
                        onmouseover="this.style.background='rgba(0,0,0,.3)'"
                        onmouseout="this.style.background='rgba(0,0,0,0)'">
                        <i class="bi bi-zoom-in" style="color:#fff;font-size:20px;"></i>
                    </div>
                </a>
                @endforeach
            </div>
