...
@section('content')
    <form method="POST" action="{{ route('admin.su-kien.update', $suKien->ma_su_kien) }}" enctype="multipart/form-data" id="suKienForm">
        ...
        <div class="create-event">
            ...
            <div class="hero-card">
                <h2>Chỉnh sửa Sự Kiện & Bố Cục</h2>
                        ...
                        <div class="form-grid">
                            <div class="span-12">
                                ...
                                <input type="text" class="form-control @error('ten_su_kien') border-danger @enderror" id="ten_su_kien" name="ten_su_kien"
                                    value="{{ old('ten_su_kien', $suKien->ten_su_kien) }}" placeholder="Ví dụ: Lễ hội công nghệ TECHX" required>
                                @error('ten_su_kien')
                                ...
                                <select class="form-control @error('ma_loai_su_kien') border-danger @enderror" id="ma_loai_su_kien" name="ma_loai_su_kien" required>
                                    ...
                                    @foreach ($loai as $item)
                                        <option value="{{ $item->ma_loai_su_kien }}"
                                            @selected(old('ma_loai_su_kien', $suKien->ma_loai_su_kien) == $item->ma_loai_su_kien)>{{ $item->ten_loai }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="span-6">
                                ...
                                <input type="text" class="form-control @error('dia_diem') border-danger @enderror" id="dia_diem" name="dia_diem"
                                    value="{{ old('dia_diem', $suKien->dia_diem) }}" placeholder="VD: Hội trường A, Nhà E" required>
                                @error('dia_diem')
                            ...
                            <div class="span-6">
                                ...
                                <input type="datetime-local" class="form-control @error('thoi_gian_bat_dau') border-danger @enderror" id="thoi_gian_bat_dau"
                                    name="thoi_gian_bat_dau" value="{{ old('thoi_gian_bat_dau', $suKien->thoi_gian_bat_dau?->format('Y-m-d\TH:i')) }}" required>
                            ...
                            <div class="span-6">
                                ...
                                <input type="datetime-local" class="form-control @error('thoi_gian_ket_thuc') border-danger @enderror" id="thoi_gian_ket_thuc"
                                    name="thoi_gian_ket_thuc" value="{{ old('thoi_gian_ket_thuc', $suKien->thoi_gian_ket_thuc?->format('Y-m-d\TH:i')) }}" required>
                            ...
                            <div class="span-6">
                                ...
                                <input type="number" min="1" class="form-control @error('so_luong_toi_da') border-danger @enderror" id="so_luong_toi_da"
                                    name="so_luong_toi_da" value="{{ old('so_luong_toi_da', $suKien->so_luong_toi_da) }}" placeholder="VD: 300" required>
                            ...
                            <div class="span-6">
                                ...
                                <input type="number" min="0" class="form-control @error('diem_cong') border-danger @enderror" id="diem_cong" name="diem_cong"
                                    value="{{ old('diem_cong', $suKien->diem_cong) }}" placeholder="VD: 5" required>
                                @error('diem_cong')
                                    <span class="text-danger" style="font-size: 0.8rem; margin-top: 4px; display: block;">{{ $message }}</span>
                                @enderror
                            </div>
        ...
        <div class="submit-bar">
            ...
            <div style="display:flex; gap:var(--space-sm);">
                ...
                <button type="submit" class="btn btn-primary" id="submitBtn" style="min-width: 160px;">
                    <i class="bi bi-check2-circle"></i> Cập nhật sự kiện
                </button>
            </div>
        </div>
    </form>
...
@section('scripts')
    ...
    <script>
        ...
        const initialDescription = @json(old('mo_ta_chi_tiet', $suKien->mo_ta_chi_tiet));
        const initialCover = @json($suKien->anh_su_kien ? asset('storage/' . $suKien->anh_su_kien) : null);
        const eventId = @json($suKien->ma_su_kien);
        ...
        form.addEventListener('submit', async e => {
            ...
            try {
                const res = await fetch('{{ route('admin.su-kien.check-collision') }}', {
                    ...
                    body: JSON.stringify({ thoi_gian_bat_dau: start, thoi_gian_ket_thuc: end, dia_diem: location, bo_qua_id: eventId })
                });
                ...
                if (data.has_collision) {
                    ...
                    if (!confirm(`Phát hiện TRÙNG LỊCH tại địa điểm này:\n\n${detail}\n\nBạn vẫn muốn tạo sự kiện này bất chấp trùng lịch?`)) {
                        ...
                        submitBtn.innerHTML = '<i class="bi bi-check2-circle"></i> Cập nhật sự kiện';
                        return;
                    }
                    
                    // Add force_update field to bypass backend verification
