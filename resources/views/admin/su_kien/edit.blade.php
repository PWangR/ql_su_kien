    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            <div class="card mb-4">
                <div class="card-body p-4">
                    <div class="form-section-title">
                        <i class="bi bi-info-circle-fill text-primary"></i> Chỉnh sửa nội dung
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="ten_su_kien" class="form-control form-control-lg @error('ten_su_kien') is-invalid @enderror"
                            placeholder="Nhập tiêu đề sự kiện..." value="{{ old('ten_su_kien', $suKien->ten_su_kien) }}" required>
                    ...
                    <div class="mb-3">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <button type="button" class="btn btn-sm btn-outline-primary shadow-sm" data-bs-toggle="modal" data-bs-target="#templateModal">
                                <i class="bi bi-magic"></i> Đổi sang mẫu khác
                            </button>
                        </div>
                        <input type="hidden" name="mo_ta_chi_tiet" id="mo_ta_chi_tiet" value="{{ old('mo_ta_chi_tiet', $suKien->mo_ta_chi_tiet) }}">
                        <div id="editor-container" style="background:#fff;">{!! old('mo_ta_chi_tiet', $suKien->mo_ta_chi_tiet) !!}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-body p-4">
                    <div class="form-section-title">
                        <i class="bi bi-images text-success"></i> Quản lý hình ảnh
                    </div>
                    
                    <div class="mb-4">
                        <label class="form-label fw-bold small text-uppercase">Ảnh bìa chính hiện tại</label>
                        <div class="row">
                            <div class="col-md-5">
                                <div class="image-upload-wrapper mb-3" onclick="document.getElementById('imgInput').click()">
                                    ...
                                    <p class="mb-0 small fw-600">Thay đổi ảnh bìa</p>
                            ...
                            <div class="col-md-7">
                                <div id="imgPreview" class="text-center">
                                    <img id="previewImg" src="{{ $suKien->anh_su_kien ? asset('storage/'.$suKien->anh_su_kien) : '' }}" 
                                        style="width:100%; max-height:180px; object-fit:cover; border-radius:12px; border:1px solid var(--border); display: {{ $suKien->anh_su_kien ? 'block':'none' }}">
                                    <span id="media_selected_text" class="d-block mt-2 small text-muted">Bỏ trống để giữ ảnh bìa cũ</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="gallery-preview mb-4" id="gallery-existing">
                            @foreach($suKien->media->where('loai_tep', 'hinh_anh') as $img)
                            <div class="gallery-item shadow-sm" id="media-{{ $img->ma_phuong_tien }}">
                                <img src="{{ asset('storage/'.$img->duong_dan_tep) }}">
                                <button type="button" class="remove-btn" onclick="removeGalleryImage({{ $img->ma_phuong_tien }})">Xóa</button>
                            </div>
                            @endforeach
                        </div>
                        ...
                        <div class="image-upload-wrapper" onclick="document.getElementById('galleryInput').click()">
                            ...
                            <p class="mb-0 small fw-600">Thêm ảnh mới vào bộ sưu tập</p>
                            <input type="file" name="gallery_files[]" class="d-none" accept="image/*" id="galleryInput" multiple>
                        </div>
                        <div id="gallery-preview-new" class="gallery-preview">
                            <!-- JS prepend new items here -->
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="sticky-actions">
                <div class="card mb-4 py-2 px-3 bg-white d-flex flex-row align-items-center justify-content-between shadow-sm border-0">
                    <span class="fw-bold text-secondary">
                        ...
                        <span class="badge bg-{{ $suKien->trang_thai_color }} ms-2">{{ $suKien->trang_thai_label }}</span>
                    </span>
                    <div class="d-flex gap-2">
                        <button type="submit" form="mainForm" class="btn btn-primary px-4 shadow-sm"><i class="bi bi-save"></i> Lưu</button>
                ...
                <div class="card mb-4">
                    <div class="card-body p-4">
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-bold small">Trạng thái sự kiện</label>
                            <select name="trang_thai" class="form-select fw-600">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="dia_diem" class="form-control border-start-0" 
                                    placeholder="Địa điểm..." value="{{ old('dia_diem', $suKien->dia_diem) }}">
                            </div>
                ...
                <div class="card">
                    <div class="card-body p-4">
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-bold small">Bắt đầu <span class="text-danger">*</span></label>
                        ...
                        <div>
                            <label class="form-label fw-bold small">Kết thúc <span class="text-danger">*</span></label>
...
document.getElementById('imgInput').addEventListener('change', function() {
    ...
    if (file) {
        ...
        reader.onload = e => {
            ...
            document.getElementById('media_selected_text').innerText = 'Dùng ảnh mới tải lên';
        };
...
const galleryPreviewNew = document.getElementById('gallery-preview-new');
...
galleryInput.addEventListener('change', function() {
    Array.from(this.files).forEach(file => {
        ...
        reader.onload = e => {
            ...
            div.innerHTML = `<img src="${e.target.result}"><button type="button" class="remove-btn" onclick="this.parentElement.remove()">Xóa</button>`;
            galleryPreviewNew.appendChild(div);
        };
        reader.readAsDataURL(file);
    });
});

// Xóa ảnh hiện tại qua AJAX
...
function selectTemplate(content, diaDiem, soLuong, diemCong, anhSuKien) {
    ...
    if (anhSuKien) {
        ...
        document.getElementById('previewImg').src = anhSuKien.startsWith('http') ? anhSuKien : "{{ asset('storage') }}/" + anhSuKien;
        document.getElementById('previewImg').style.display = 'block';
        document.getElementById('media_selected_text').innerText = 'Thay bằng ảnh từ mẫu';
    }
...
function selectMedia(path, element) {
    ...
    document.getElementById('previewImg').style.display = 'block';
    document.getElementById('media_selected_text').innerText = 'Thay bằng ảnh từ thư viện';
...
function saveLoaiSuKien() {
    fetch('{{ route("admin.su-kien.store-loai") }}', {
        method: 'POST',
        headers: {'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}'},
        body: JSON.stringify({ ten_loai: document.getElementById('new_loai_ten').value, mo_ta: document.getElementById('new_loai_mota').value })
    }).then(res => res.json()).then(data => {
        if(data.success) {
            const sel = document.getElementById('ma_loai_su_kien');
            sel.add(new Option(data.loai.ten_loai, data.loai.ma_loai_su_kien, true, true));
