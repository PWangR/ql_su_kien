...
<div style="margin-bottom: var(--space-lg);">
    <form method="GET" style="display: flex; flex-wrap: wrap; gap: var(--space-sm); align-items: center;">
        <label style="font-size: 0.875rem; font-weight: 600; color: var(--text-muted); width: 100%; margin-bottom: 4px;">Trạng thái:</label>
        @foreach(collect(request()->query())->except(['trang_thai', 'page']) as $key => $value)
        <input type="hidden" name="{{ $key }}" value="{{ $value }}">
        @endforeach
        <select name="trang_thai" style="padding: 8px 12px; border: 1px solid var(--border); border-radius: var(--border-radius); background: var(--card); color: var(--text); font-size: 0.875rem; cursor: pointer; flex: 1; min-width: 180px;" onchange="this.form.submit();">
...
<div class="filter-tabs" style="border-bottom:1px solid var(--border);margin-bottom:var(--space-lg);display:flex;overflow-x:auto;">
    ...
    @endphp
    <a href="{{ route('events.index', $baseQuery) }}"
        class="filter-tab {{ !request('loai') ? 'active' : '' }}"
...
<div class="events-grid">
    ...
    <div class="event-card">
        <div class="event-card-img">
            ...
            'da_ket_thuc' => ['label' => 'Đã kết thúc', 'badge' => 'badge-secondary'],
            ];
